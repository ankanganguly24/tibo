...
const commands = [
  ["Install and scan", "npx --yes @goankan/tibo@0.1.1 scan", "Run from the repository the agent changed."],
  ["Machine output", "npx --yes @goankan/tibo@0.1.1 scan --json", "Give stable findings to an agent, script, or CI job."],
  ["Record a decision", "npx --yes @goankan/tibo@0.1.1 decide <id> keep|reject|later", "Store only the human's explicit choice."],
  ["Show project memory", "npx --yes @goankan/tibo@0.1.1 summary --json", "Give the next agent the ledger and unresolved findings."],
] as const;
...
cp -R skills/tibo-review/* \\
  .claude/skills/tibo-review/`}</CodeBlock></div></div><div className="mt-8 rounded-md border border-line bg-[#10100f] p-6 sm:p-8"><p className="font-mono text-xs text-amber">The host-agent contract</p><ol className="mt-5 space-y-4 text-base leading-7 text-[#aaa69d]"><li><span className="mr-3 font-mono text-paper">1.</span>Run <code>npx --yes @goankan/tibo@0.1.1 scan --json</code> after the agent changes files.</li><li><span className="mr-3 font-mono text-paper">2.</span>Show each summary, evidence path, confidence, and limitation.</li><li><span className="mr-3 font-mono text-paper">3.</span>Ask the human for keep, reject, or later. Never choose silently.</li><li><span className="mr-3 font-mono text-paper">4.</span>Record the explicit answer with <code>npx --yes @goankan/tibo@0.1.1 decide</code>.</li><li><span className="mr-3 font-mono text-paper">5.</span>Only after separate approval, make a narrow repair, test it, and scan again.</li></ol></div></div></section>
